...
export function IntersectDialog({ open, onOpenChange }: IntersectDialogProps): ReactElement {
  ...
  function safeIntersect(
    ...
    try {
      return turf.intersect(a as any, b as any) as Feature<Polygon | MultiPolygon> | null
    } catch (err) {
  ...
  function handleIntersect(layerIds: string[], outName: string): boolean {
    const layerPolys: Feature<Polygon | MultiPolygon>[][] = layerIds.map((id) => {
...
function IntersectTool({
  ...
  const toggle = (id: string, name: string) => {
    setSelectedLayerIds((prev) => {
      ...
      if (next.length && !name.includes("-intersect")) setLayerName(getUniqueLayerName(`${name}-intersect`))
      return next
    })
  }
  ...
  return (
    <div className="mt-2 mb-4 ml-1 mr-1 flex flex-row items-center gap-2">
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent className="w-[230px] p-0" aria-label="Polygon layer list">
          <Command>
            ...
            <CommandList>
              ...
              <CommandGroup>
                {polyLayers.map((layer) => {
                  ...
                  return (
                    <CommandItem key={layer.id} value={layer.name} onSelect={() => toggle(layer.id, layer.name)}>
      ...
      {errors.layers && <p className="text-sm text-red-500">Select at least two polygon layers.</p>}
    </div>
  )
}